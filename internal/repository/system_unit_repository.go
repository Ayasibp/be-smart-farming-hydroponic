...
func (r systemUnitRepository) CreateSystemUnit(inputModel *model.SystemUnit) (*model.SystemUnit, error) {
	res := r.db.Raw("INSERT INTO hydroponic_system.system_units (farm_id , tank_volume , tank_a_volume , tank_b_volume , created_at) VALUES (?,?,?,?,?) RETURNING *;", inputModel.FarmId, inputModel.TankVolume, inputModel.TankAVolume,inputModel.TankBVolume, time.Now()).Scan(inputModel)
