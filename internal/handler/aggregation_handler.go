...
func (h *AggregationHandler) CreateBatchAggregationGrowthHist(c *gin.Context) {
	logger.Info("aggregationHandler", "Starting batch aggregation growth history process")

	resp, err := h.aggregationService.CreateBatchGrowthHistMonthlyAggregation()
	if err != nil {
		logger.Error("aggregationHandler", "Failed to create batch aggregation growth history", "error", err.Error())
	...
	if err != nil {
		logger.Error("aggregationHandler", "Failed to create system log for batch aggregation", "error", err.Error())
		response.Error(c, 400, err.Error())
		return
	}

	logger.Info("aggregationHandler", "Successfully created batch aggregation growth history")
...
func (h *AggregationHandler) CreateCurrentMonthAggregationGrowthHist(c *gin.Context) {
	logger.Info("aggregationHandler", "Starting current month aggregation growth history process")

	resp, err := h.aggregationService.CreatePrevMonthAggregation()
	if err != nil {
		logger.Error("aggregationHandler", "Failed to create current month aggregation growth history", "error", err.Error())
		response.Error(c, 400, err.Error())
		return
	}
	...
	if err != nil {
		logger.Error("aggregationHandler", "Failed to create system log for monthly aggregation", "error", err.Error())
		response.Error(c, 400, err.Error())
		return
	}

	logger.Info("aggregationHandler", "Successfully created current month aggregation growth history")
